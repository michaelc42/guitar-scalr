...
export const Fretboard = (props) => {
    ...
    const { numberOfFrets, numberOfStrings, openStrings, scale } = props;
    ...
    const fretWidthCalc = (totalFrets = 12) => {
        ...
        for (let i = 0; i < totalFrets; i++) {
            ...
            fretWidths.push(currWidth.toFixed(3))
            scaleLength -= prevWidth
        }
        return fretWidths
    }

    fretWidthRatios = fretWidthCalc().map(x => x * (100 / 13))
    
    console.log(fretWidthRatios)
    ...
    const setupFrets = (stringNum, startNote) => {
        ...
        for (let i = 0; i < numberOfFrets + 1; i++) {
            ...
            document.documentElement.style.setProperty(`--fret-width`, currFretWidthRatio)
